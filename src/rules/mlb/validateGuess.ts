...
function assignmentPassesCellRules(
  ...
  if (assignment.stats.position !== puzzle.position) {
    return {
      valid: false,
      failures: [
        {
...
function findEasyModeAssignments(
  ...
  for (const seasonRow of db.getSeasonsForPlayer(playerId, puzzle.position)) {
    const assignment = db.getAssignment(
      playerId,
      seasonRow.season,
      puzzle.position,
    );
...
export function validateGuess(
  ...
  if (mode === "hard") {
    ...
    for (const playerId of playerIds) {
      const assignment = db.getAssignment(playerId, season, puzzle.position);
      if (!assignment) {
        continue;
      }
    ...
    if (passing.length > 1) {
      const names = [...new Set(passing.map((a) => a.playerName))];
      return {
        valid: false,
        failures: [
          {
  ...
  for (const playerId of playerIds) {
    validAssignments.push(
      ...findEasyModeAssignments(db, playerId, input.cellId, puzzle, board),
    );
  }
