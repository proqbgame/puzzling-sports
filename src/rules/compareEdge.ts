...
function getStatValue(assignment: PlayerSeasonAssignment, stat: StatKey): number {
  return assignment.stats[stat];
}

/**
 * Compare `from` vs `to` on the edge they share.
 *
 * @param connectionOnFrom - tab or socket on the `from` cell's side of the edge
 */
