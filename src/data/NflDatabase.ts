...
export class NflDatabase {
  ...
  /**
   * Normalize season input like "2023" (NFL year labels).
   * Also accepts "2023-24" / "2023-2024" and maps to the start year when present.
   */
  ...
  /** QB-only player-season assignments (filters shared QB+WR+RB dataset). */
  getQbAssignments(): readonly PlayerSeasonAssignment[] {
    return this.getAllAssignments().filter(
      (assignment) => (assignment.stats.position ?? "QB") === "QB",
    );
  }

  /** WR-only player-season assignments. */
  getWrAssignments(): readonly PlayerSeasonAssignment[] {
    return this.getAllAssignments().filter(
      (assignment) => assignment.stats.position === "WR",
    );
  }

  /** RB-only player-season assignments. */
  getRbAssignments(): readonly PlayerSeasonAssignment[] {
    return this.getAllAssignments().filter(
      (assignment) => assignment.stats.position === "RB",
    );
  }
}
