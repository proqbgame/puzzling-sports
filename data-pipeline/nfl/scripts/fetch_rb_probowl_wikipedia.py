"""
Build / resume proBowlRb seed entries from Wikipedia Pro Bowl pages.
...
OUT_PATH = PIPELINE_DIR / "cache" / "awards" / "proBowlRb_wikipedia.json"
...
RB_POSITION_LABELS = {
    ...
    "rb",
    "halfback",
    "fullback",  # include FB listed on older Pro Bowl offense tables if any
}
...
def _extract_rb_names(html: str) -> list[str]:
    names: list[str] = []
    ...
    for row in rows:
        ...
        if pos not in RB_POSITION_LABELS:
            continue
        # Skip dedicated fullback-only rows for puzzle RB pool? Keep them —
        # FB Pro Bowl selections are rare and usually not in our RB volume set.
        if pos == "fullback":
            continue
        blob = " ".join(cells[1:])
...
def main() -> None:
    ...
    by_season: dict[str, list[str]] = dict(seed.get("proBowlRb") or {})
    errors: list[str] = []

    for event_year in EVENT_YEARS:
        season = str(event_year - 1)
        if season in by_season and by_season[season]:
            print(f"{season}: skip (already have {len(by_season[season])} RBs)")
            continue

        title = _page_title(event_year)
        try:
            html = _fetch_with_retry(title)
            names = _extract_rb_names(html)
            if not names and event_year > 2022:
                html = _fetch_with_retry(f"{event_year}_Pro_Bowl")
                names = _extract_rb_names(html)
            if not names:
                errors.append(f"{season}: no RBs found on {title}")
                print(f"{season}: EMPTY")
            else:
                by_season[season] = names
                print(f"{season}: {len(names)} RBs from {title}")
                seed["proBowlRb"] = dict(sorted(by_season.items(), key=lambda kv: int(kv[0])))
    ...
    OUT_PATH.write_text(
        json.dumps({"proBowlRb": seed["proBowlRb"], "errors": errors}, indent=2),
        encoding="utf-8",
    )
    print(f"\nSeasons filled: {len(seed['proBowlRb'])} / {len(EVENT_YEARS)}")
    if errors:
        print("Errors:")
        for e in errors:
            print(" ", e)
